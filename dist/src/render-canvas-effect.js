export function renderCanvasEffect(canvasContext) {
    ...
    function drawGridOfLines(gridCellSize, lineLength) {
        for (let y = -gridCellSize; y < height + gridCellSize * 2; y += gridCellSize) {
            for (let x = -gridCellSize; x < width + gridCellSize * 2; x += gridCellSize) {
                const angleMultiplier = 20;
                const angle = (x / gridCellSize) * angleMultiplier +
